...
const AssignmentPage = ({ user }) => {
  ...
  const fetchMyResults = async () => {
    try {
      ...
      if (!res.ok) throw new Error('Failed to load my results');
      const data = await res.json();
      console.log('Fetched my results:', data);
  ...
  const calculateDashboardStats = () => {
    ...
    try {
      ...
      results.forEach(r => {
        ...
        if (!byMode[mode]) {
          byMode[mode] = { count: 0, totalScore: 0, totalMax: 0 };
        }
        byMode[mode].count += 1;
        byMode[mode].totalScore += r.score || 0;
        byMode[mode].totalMax += r.total || 1;
      });

      const byDeck = {};
      results.forEach(r => {
        const deckName = r.deck_name || r.deck_id || 'All Decks';
        if (!byDeck[deckName]) {
          byDeck[deckName] = { count: 0, totalScore: 0, totalMax: 0 };
        }
        byDeck[deckName].count += 1;
        byDeck[deckName].totalScore += r.score || 0;
        byDeck[deckName].totalMax += r.total || 1;
      });

      Object.keys(byMode).forEach(mode => {
        const total = byMode[mode].totalMax || 1;
        byMode[mode].avgScore = ((byMode[mode].totalScore / total) * 100).toFixed(1);
      });

      Object.keys(byDeck).forEach(deck => {
        const total = byDeck[deck].totalMax || 1;
        byDeck[deck].avgScore = ((byDeck[deck].totalScore / total) * 100).toFixed(1);
      });
  ...
  const renderDashboard = () => {
    try {
      ...
      const scores = results.map(r => Math.round(((r.score || 0) / (r.total || 1)) * 100));
        ...
        datasets: [
          {
            ...
            data: binCounts,
            backgroundColor: ['#0e81c8a8', '#357ab8', '#4fc3f7', '#81d4fa', '#b3e5fc', '#e1f5fe'],
            borderRadius: 8,
            borderWidth: 2,
            borderColor: '#fff',
      ...
      // Use groupMembers as the student list
      const allStudents = groupMembers && Array.isArray(groupMembers) && groupMembers.length > 0 ? groupMembers : [];
      const takenIds = new Set(results.map(r => r.user_id));
      const notTaken = allStudents.filter(s => s && s.id && !takenIds.has(s.id));
      const notPassed = results.filter(r => r && r.score !== undefined && r.total && ((r.score / r.total) * 100 < 50));
      ...
      return (
        <div className="dashboard-content">
          ...
          {allStudents.length === 0 ? (
            ...
            <div className="dashboard-section">
              <h4 style={{ color: '#e74c3c' }}>Students who didn't take the test ({notTaken.length})</h4>
              <ul style={{ color: '#e74c3c', fontWeight: 600 }}>
                {notTaken.map(s => (
                  <li key={s.id}>{transliterate(String(s.id))}</li>
                ))}
              </ul>
            </div>
          ) : (
          ...
          {notPassed.length > 0 ? (
            <div className="dashboard-section">
              <h4 style={{ color: '#ff9800' }}>Students who didn't pass (score &lt; 50%) ({notPassed.length})</h4>
              <ul style={{ color: '#ff9800', fontWeight: 600 }}>
                {notPassed.map(r => (
                  <li key={r.id}>{transliterate(String(r.username || r.user_id))} ({r.score}/{r.total})</li>
                ))}
              </ul>
            </div>
          ) : (
          ...
          {/* Overall group student list */}
          <div className="dashboard-section">
            <h4 style={{ color: '#357ab8' }}>Overall Group Student List ({allStudents.length})</h4>
            {allStudents.length === 0 ? (
              ...
              <table className="asn-results-table">
                ...
                <tbody>
                  {Object.entries(stats.byMode).map(([mode, data]) => (
                    <tr key={mode}>
                      <td>{transliterate(String(mode))}</td>
                      <td>{data.count}</td>
                      <td>{data.avgScore}%</td>
                    </tr>
                  ))}
              ...
              <table className="asn-results-table">
                ...
                <tbody>
                  {Object.entries(stats.byDeck).map(([deck, data]) => (
                    <tr key={deck}>
                      <td>{transliterate(String(deck))}</td>
                      <td>{data.count}</td>
                      <td>{data.avgScore}%</td>
                    </tr>
                  ))}
      ...
      return <p className="gp-empty">Error rendering dashboard. Please refresh the page.</p>;
    }
  };
    ...
    <>
      ...
      <div className="sg-cont">
        ...
        <div className="sg-block gp-main">
          ...
          <p className="gp-desc">
            {assignment.due_date ? `${t('due')}: ${new Date(assignment.due_date).toLocaleString()}` : t('noDueDate')}
          </p>

          {assignmentExpired && (
            <div
              className="eg-error"
              style={{ backgroundColor: '#f8d7da', color: '#721c24', border: '1px solid #f5c6cb', padding: '10px', borderRadius: '13px', marginBottom: '10px' }}
            >
          ...
          <div className="asn-tabs">
            <button className={`asn-tab ${activeTab === 'overview' ? 'asn-tab-active' : ''}`} onClick={() => setActiveTab('overview')}>
              {t('assignmentDecks')}
            </button>
            <button className={`asn-tab ${activeTab === 'results' ? 'asn-tab-active' : ''}`} onClick={() => setActiveTab('results')}>
              {isTeacher ? t('allResults') : t('yourResults')}
            </button>
            <button className={`asn-tab ${activeTab === 'leaderboard' ? 'asn-tab-active' : ''}`} onClick={() => setActiveTab('leaderboard')}>
              {t('leaderboard')}
            </button>
            {isTeacher && (
              <button className={`asn-tab ${activeTab === 'dashboard' ? 'asn-tab-active' : ''}`} onClick={() => setActiveTab('dashboard')}>
              ...
              <div className="gp-items">
                {(assignment.decks || []).map(d => (
                  <div key={d.id} className="asn-deck-row">
                    <span className="group-link" style={{ fontWeight: 700 }}>
                      {d.name}
                    </span>
                  </div>
                ))}
                ...
                <table className="asn-results-table">
                  ...
                  <tbody>
                    {displayResults.map(r => (
                      <tr key={r.id}>
                        {isTeacher && <td>{transliterate(String(r.username || r.user_id))}</td>}
                      ...
                      <tr key={entry.user_id}>
                        ...
                        <td>
                          <Link className="group-link" to={`/profile/${entry.user_id}`}>
                            {transliterate(String(entry.username || entry.user_id))}
                          </Link>
                        </td>
