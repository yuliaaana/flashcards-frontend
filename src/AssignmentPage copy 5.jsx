...
const AssignmentPage = ({ user }) => {
  ...
  const fetchMyResults = async () => {
    try {
      ...
      const data = await res.json();
      console.log('Fetched my results:', data); // Debug log
  ...
  const renderDashboard = () => {
    try {
      ...
      const notPassed = results.filter(r => ((r.score || 0) / (r.total || 1)) * 100 < 50);
          ...
          {notPassed.length > 0 ? (
            <div className="dashboard-section">
              ...
              <ul style={{ color: '#ff9800', fontWeight: 600 }}>
                {notPassed.map(r => (
                  <li key={r.id}>{transliterate(r.username || r.user_id)} ({r.score}/{r.total})</li>
                ))}
              </ul>
            </div>
          ) : (
