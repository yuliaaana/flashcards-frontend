...
const GroupPage = ({ user }) => {
  ...
  const [selectedDashboardAsn, setSelectedDashboardAsn] = useState('all');
  ...
  const fetchDashboard = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/groups/${groupId}/dashboard`);
      if (!res.ok) throw new Error();
      const data = await res.json();
  ...
  // --- Dashboard helpers ---
  const dashboardFiltered = dashboardData.filter(r => {
    if (selectedDashboardAsn === 'all') return true;
    return r.assignment_results?.some(ar => ar.assignment_id === parseInt(selectedDashboardAsn));
  });

  const dashAvgCompletion = dashboardData.length
    ? Math.round(
        dashboardData.reduce((s, r) => s + (assignments.length ? r.assignments_done / assignments.length : 0), 0)
        / dashboardData.length * 100
      )
    : 0;

  const dashAvgScore = dashboardData.length
          ...
          <div className="gp-assignments-section">
            ...
            {isTeacher && !showCreateAssignment && (
              <div className="gp-teacher-actions">
                <button
                  className="gp-add-btn btn-cr-ass"
              ...
              <div className="gp-dashboard">
                ...
                <div className="dashboard-stats">
                  <div className="dash-stat">
                    <span className="dash-stat-label">{t('students') || 'Students'}</span>
                    <span className="dash-stat-val">{members.length}</span>
                  </div>
                  <div className="dash-stat">
                    <span className="dash-stat-label">{t('assignments') || 'Assignments'}</span>
                    <span className="dash-stat-val">{assignments.length}</span>
                  </div>
                  <div className="dash-stat">
                    <span className="dash-stat-label">{t('avgCompletion') || 'Avg completion'}</span>
                    <span className="dash-stat-val">{dashAvgCompletion}%</span>
                  </div>
                  <div className="dash-stat">
                    <span className="dash-stat-label">{t('avgScore') || 'Avg score'}</span>
                    <span className="dash-stat-val">{dashAvgScore}%</span>
                  </div>
                </div>
                ...
                <select
                  ...
                  value={selectedDashboardAsn}
                  onChange={e => setSelectedDashboardAsn(e.target.value)}
                >
                  <option value="all">{t('allAssignments') || 'All assignments'}</option>
                  {assignments.map(a => (
                ...
                {dashboardLoading ? (
                  <p className="gp-empty">{t('loading')}</p>
                ) : dashboardFiltered.length === 0 ? (
                  <p className="gp-empty">{t('noData') || 'No data available yet.'}</p>
                ) : (
                      ...
                      <tbody>
                        {dashboardFiltered.map(r => {
                          ...
                          return (
                            <tr key={r.user_id}>
                              <td>
                                <Link className="group-link" to={`/profile/${r.user_id}`}>
                                  {r.username}
                                </Link>
                              </td>
